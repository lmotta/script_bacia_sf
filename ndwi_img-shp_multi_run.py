...
"""
/***************************************************************************
Name                 : ndwi_img-shp_multi_run
Description          : Create set of files(image and shapefile) with NDWI values
Arguments            : list of images, band1, band2, limits(separate by comma), 
...
def apply_ndwi(params, l_pathfile):
  ...
  log = LogProcess( params['log_file'], start_end=False )
  for id in xrange( len( l_pathfile ) ):
    createNDWI( l_pathfile[ id ], log )
...
def run(pathfile, numB1, numB2, limits, min_pixels, dirOut):
  ...
  log_file = "ndwi_img-shp_multi_run.log"
  ...
  ll_img = getListImages( cpu_count )
  pool = mp.Pool()
  for id in xrange( cpu_count ):
    pool.apply_async(
      func=apply_ndwi,
      args=( params, ll_img[ id ] )
    )
  pool.close()
  pool.join()
